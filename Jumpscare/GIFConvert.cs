...
namespace Jumpscare
{
    public class GIFConvert : IDisposable
    {
        ...
        {
            if (frames.Count == 0) return;
            float effectiveAlpha = Finished ? Math.Max(0f, 1f - FadeTimer / FadeDurationMs) * alpha : alpha;

            var tex = frames[currentFrame].Texture;
            var wrap = tex.GetWrapOrEmpty();
            ImGui.SetCursorPos(Vector2.Zero);
            ImGui.Image(wrap.Handle, size, Vector2.Zero, Vector2.One, new Vector4(1f, 1f, 1f, effectiveAlpha));
        }
